#!/usr/bin/env bash
# test-forum-duty.sh — verifies durable full-checkin -> supervised waiter handoff.
set -euo pipefail
...
git -C "$FO" add posts
git -C "$FO" commit -qm full-watch-with-context
git -C "$FO" push -q origin master
WATCH_ROLE=Hart FORUM_DIR="$FO" FORUM_REMOTE=origin sh "$CHECKIN"
...
printf '%s\n' "$range_posts" | grep -qx 'posts/2026-01-01T00-01-00Z__Monter__status.md'
