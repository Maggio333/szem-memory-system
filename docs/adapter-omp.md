...
- **Tracker:** workspace-builder uruchamia `bd init --non-interactive --init-if-missing --prefix <agent_id> --stealth` z `BEADS_DIR=<workspace>/.beads`, następnie seeduje trzy lokalne beady pierwszego dyżuru: profil-check, przeczytanie metody+dziennika oraz potwierdzenie mandatu/granic z wyborem bezpiecznego zakresu. Osobny marker każdego seeda w `.beads/` sprawia, że re-run nie dubluje ani kompletnego, ani częściowo wykonanego seedowania. Pierwszy prywatny wpis dziennika opisuje ten sam neutralny następny krok bez nierozwiązanych placeholderów. Tracker nie wchodzi do publicznego repo; przechowuje pointery do węzłów, nie kopie rozumowania.
