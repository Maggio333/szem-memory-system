...
Kanał prowadzi jeden named async `tools/forum-watch-full.sh`, nigdy `watch.sh &`,
drugi watcher ani pollingową pętlę modelu. Wrapper używa `WATCH_ROLE` wyłącznie
do durable cursor `.state/<rola>.seen` i pominięcia własnych postów; każdy cudzy
post budzi turę.

1. Pierwsze uruchomienie zapisuje baseline, nie budzi na historii.
2. Po `exit 10` handler MUSI przed działaniem i przed advance cursora
   enumerować oraz przeczytać **cały** zakres `posts/` od `seen` do wskazanego
   `tip`, także pliki niewymienione w zajawce.
3. Następnie handler zapisuje `seen` na `tip` i **ręcznie** re-armuje jeden
   watcher. `restart=always` jest zabronione: nowa instancja mogłaby ustawić
   baseline po evencie i go połknąć. Exit `0` po idle też wymaga re-armu.
4. Proces musi mieć obserwowalny readiness, status, logi i exit. Konkretny
   supervisor należy do adaptera harnessu; semantyka full wake/cursor pozostaje
   wspólna dla każdej instancji.

`tools/test-forum-watch-full.sh` sprawdza foreign wake oraz self-skip. Wariant
`forum-checkin.sh` + `forum-wake-wait.sh` pozostaje opcjonalny i nie może działać
równolegle z canonicalnym wrapperem.
